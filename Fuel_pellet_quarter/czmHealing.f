...
c                    
     ...
     $     dpred(*),statev(nstatv)
c                       
		 IF (temp(1).LE.600) Then
			expanmat = 9.84e-6
		 ELSE IF (temp(1).LE.800) Then
			expanmat = 9.84e-6+0.09e-6*((temperature-600)/200)
		 ELSE IF (temp(1).LE.1000) Then
			expanmat = 9.93e-6+0.07e-6*((temperature-800)/200)	
		 ELSE IF (temp(1).LE.1200) THEN
			expanmat = 10.e-6+0.2e-6*((temperature-1000)/200)
		 ELSE IF (temp(1).LE.1400) THEN
			expanmat = 10.2e-6+0.3e-6*((temperature-1200)/200)
		 ELSE IF (temp(1).LE.1600) THEN
			expanmat = 10.5e-6+0.3e-6*((temperature-1400)/200)
		 ELSE IF (temp(1).LE.1800) THEN
			expanmat = 10.8e-6+0.5e-6*((temperature-1600)/200)
		 ELSE IF (temp(1).LE.2000) THEN
			expanmat = 11.3e-6+0.5e-6*((temperature-1800)/200)
		 ELSE IF (temp(1).LE.2200) THEN
			expanmat = 11.8e-6+0.6e-6*((temperature-2000)/200)
		 ELSE IF (temp(1).LE.2400) THEN
			expanmat = 12.4e-6+0.7e-6*((temperature-2200)/200)
		 ELSE IF (temp(1).LE.2600) THEN
			expanmat = 13.1e-6+0.8e-6*((temperature-2400)/200)
		 ELSE IF (temp(1).LE.2800) THEN
			expanmat = 13.9e-6+0.9e-6*((temperature-2600)/200)
		 ELSE 
			expanmat = 14.8e-6
		 ENDIF 
		 expangonfl = 4.e-9
...
c
c		 -----PASTILLE-GAINE-----
		 IF (SLNAME(1:16) .EQ. 'ASSEMBLY_SURF-1' .AND. MSNAME(1:16) .EQ. 'ASSEMBLY_SURF-2') THEN
c 		 	-------Conductivite------
			conductivite_He  = 0.274
			IF (D(1).GE.1.e-7) Then
				h_cond = conductivite_He/D(1)
			ELSE 
				h_cond  = conductivite_He /1.e-10
			ENDIF
c 		 	-------Radiation------
			emissivite_MOX = 0.9
			emissivite_clad = 0.85
			const_StefanBoltzman = 5.67032e-8
			h_rad = (const_StefanBoltzman/((1/emissivite_MOX) + (1/emissivite_clad) - 1))*((TEMP(1)**4-TEMP(2)**4)/(TEMP(1)-TEMP(2)))
c		 	WRITE(*,*)"h_rad : ",TEMP(1),TEMP(2),h_rad 
			AK(1) = h_cond
c
c		 -----CZM-----
		 ELSE
...
C     INTEGRATION OF THE EVOLUTIONS (DAMAGE AND CHEMISTRY AND FRICTION) 
     ...
     2   DTIME,TIME(2),TEMP+DTEMP,TEMP,PROPS,1.D-3,NPROPS,NTENS,EPSTeOld)
      ENDIF
...
C	  lc = 2.
c	  STATEV(10)= 0.5*(damageFunc*DKplus*EPSNplus*EPSNplus+damageFunc*DKt*EPSTe(I)*EPSTe(I)+DKminus*EPSNminus*EPSNminus)
	  STATEV(11)= PROPS(7)*Da 
...
C       Correction ...
        ...
        Dk(2,1)=-(h/Eta2)*(DAh_Dda*HEAV(Eps_crit-EPSNminus))*HEAV(Dpsi_Dda)
        Dk(2,2)=1.-(h/Eta2)*(DAh_Dh*HEAV(Eps_crit-EPSNminus)*HEAV(Dpsi_Dda))
